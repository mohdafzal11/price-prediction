...
export default async function handler(
    ...
    try {
        ...
        const cacheKey = `coin_all_prediction_${id}`;
        
        // // If not forcing a refresh, try to get from cache
        if (!forceRefresh) {
          const cachedPredictions = await redisHandler.get(cacheKey) as any;
          // check if 3 day and 5 day predictions are available
          const predictions = cachedPredictions?.predictions;
          if (predictions) {
            if (cachedPredictions.yearlyPredictions){
              if (!(predictions.threeDay['confidence'] == null || predictions.fiveDay['confidence'] == null)) {
